...
// Builds a fully-wired Express app with all API routes mounted. Safe to call from both
// the local Node server (server/index.ts) and the Vercel serverless entrypoint (api/index.ts).
export function createApp(): Express {
  ...
  app.post('/api/sales/analysis', requireAuth, analysisLimiter, async (req: any, res) => {
    ...
    // On Vercel, background work after res.json may be cut off. Await the email send
    // so the function stays alive; on Node hosts this adds trivial latency.
    try {
      const [user] = await db.select().from(users).where(eq(users.id, userId));
      if (user?.email) {
        const prefs = await preferencesStore.getOrDefault(userId);
        if (prefs.followUpEmailsEnabled) {
  ...
  app.post('/api/sales/coaching', requireAuth, aiLimiter, async (req, res) => {
    const { observation, sellerName, metrics } = req.body as {
      observation: string;
      sellerName?: string;
      metrics?: { talkRatio?: number; questionScore?: number; avgBuyLikelihood?: number };
    };
  ...
  // --- Cron endpoints (Vercel Cron calls these over HTTP) --------------------
  // Protected by CRON_SECRET header. In production the secret MUST be set; when
  // unset we only accept requests in non-production (local testing).
  const requireCronAuth = (req: any, res: any, next: any) => {
    const expected = process.env.CRON_SECRET;
    if (!expected) {
      if (process.env.NODE_ENV === 'production') return res.status(401).json({ message: 'CRON_SECRET not configured.' });
      return next();
    }
    const header = req.headers.authorization || '';
    if (header !== `Bearer ${expected}`) return res.status(401).json({ message: 'Unauthorized.' });
    return next();
  };

  app.get('/api/cron/weekly-digest', requireCronAuth, async (_req, res) => {
    try {
      const r = await sendWeeklyDigestsIfDue({ force: true });
  ...
  app.get('/api/cron/blocker-reminders', requireCronAuth, async (_req, res) => {
    try {
      const r = await sendBlockerReminders();
      res.status(200).json({ ok: true, ...r });
    } catch (err) {
      console.error('[cron:blocker-reminders] failed', err);
      res.status(500).json({ ok: false, error: String(err) });
    }
  });
